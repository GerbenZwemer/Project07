...
if($_SESSION['rol'] == 2){
    ...
    foreach ($resultaat as $res) {
        ?> 
        <div class="cliëntenkaart">
            <form action="cliënten2.php" method="POST">
                <?php echo "<span class='cliëntNaam'>".$res['voornaam']; ?>
                <?php echo $res['achternaam']; ?>
                <?php echo $res['polisnummer']."</span>"; ?>
                <input type='hidden' name='cliëntid' value=<?php echo $res['id']; ?>>
                <input type='hidden' name='voornaam' value=<?php echo $res['voornaam']; ?>>
                <input type='hidden' name='achternaam' value=<?php echo $res['achternaam']; ?>><br>
                <i class="fa fa-picture-o foto" aria-hidden="true"></i><br>
                <button class='button link' type='submit' name='submit' value='info'><i class='buttonlink fa fa-info fa-2x' aria-hidden='true'></i></button>
            </form>
        </div>
        <!--
        <form>
