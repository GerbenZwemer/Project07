...
if(isset($_POST['wijzigen'])){
    $sql = "UPDATE huisartsen SET voornaam=?,tussenvoegsel=?,achternaam=?,telefoonnummer=? WHERE id =?";
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($_POST['voornaam'],$_POST['tussenvoegsel'],$_POST['achternaam'],$_POST['telefoonnummer'],$_POST['id']));
    echo 'Huisarts is gewijzigd';
    ?>
    <script>
        location.replace("huisartsaanpassen.php");
    </script>
<?php
}if(isset($_POST['verwijderen'])){
    $sql = "DELETE FROM `huisartsen` WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($_POST['id']));
    echo 'Huisarts is verwijderd';
    ?>
    <script>
        location.replace("huisartsaanpassen.php");
    </script>
