...
if(isset($_POST['wijzigen'])){
    $sql = "UPDATE specialisten SET voornaam=?,tussenvoegsel=?,achternaam=?,telefoonnummer=? WHERE id =?";
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($_POST['voornaam'],$_POST['tussenvoegsel'],$_POST['achternaam'],$_POST['telefoonnummer'],$_POST['id']));
    echo 'Medischspecialist is gewijzigd';
    ?>
    <script>
        location.replace("medischspecialistaanpassen.php");
    </script>
<?php
}if(isset($_POST['verwijderen'])){
    $sql = "DELETE FROM `specialisten` WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($_POST['id']));
    echo 'Medischspecialist is verwijderd';
    ?>
    <script>
        location.replace("medischspecialistaanpassen.php");
    </script>
