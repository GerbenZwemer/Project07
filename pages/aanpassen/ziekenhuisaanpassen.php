...
if(isset($_POST['wijzigen'])){
    $sql = "UPDATE ziekenhuizen SET naam=?,woonplaats=?,straatnaam=?,huisnummer=?,postcode=?,telefoonnummer=?,telefoonnummer=? WHERE id =?";
    $stmt = $conn->prepare($sql);
    ...
    echo 'Ziekenhuis is gewijzigd';
    ?>
    <script>
        location.replace("ziekenhuisaanpassen.php");
    </script>
<?php
}if(isset($_POST['verwijderen'])){
    $sql = "DELETE FROM `ziekenhuizen` WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->execute(array($_POST['id']));
    echo 'Ziekenhuis is verwijderd';
    ?>
    <script>
        location.replace("ziekenhuisaanpassen.php");
    </script>
